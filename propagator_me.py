...
def prop_orbit(initial_state, CustomAtmosphere, with_drag=False, plot_trajectory=True, **kwargs):
    ...
    horizon = kwargs.get('horizon', 3 * 86400.0)
    ...
    tol = NumericalPropagator.tolerances(
        positionTolerance, initial_orbit, orbitType)

    integrator = DormandPrince853Integrator(
        minStep, maxstep, JArray_double.cast_(tol[0]), JArray_double.cast_(tol[1]))
    ...
    gravityProvider = GravityFieldFactory.getConstantNormalizedProvider(
        degree, torder, initialDate)
    gravityForce = HolmesFeatherstoneAttractionModel(
        earth.getBodyFrame(), gravityProvider)
    ...
    if (plot_trajectory):
        ...
        stat_list = [horizon, toc - tic, px[-1], py[-1],
                     pz[-1], vx[-1], vy[-1], vz[-1], step]
        ...
        Z_Earth = r_Earth*1e-3 * \
            np.outer(np.ones(np.size(phi)), np.sin(theta)).T
        ax.plot_surface(X_Earth, Y_Earth, Z_Earth, cmap='binary',
                        alpha=0.35, antialiased=False, zorder=1)
