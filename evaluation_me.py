...
class DensityModelEvaluator:
    def __init__(self, ground_truth_file, participant_file):
        ...
        #     raise ValueError(
        #         "Mismatch between ground truth and participant data structure.")
    ...
    def _compute_weight(self, timestamps, epsilon=1e-5):
        ...
        """
        t_seconds = np.array(pd.to_datetime(
            timestamps) - pd.to_datetime(timestamps[0]), dtype='timedelta64[s]').astype(int)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--participant', type=str, required=True,
                        help='Path to the participant JSON file.')
