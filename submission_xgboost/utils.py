...
def create_pred_timestamps(tm):
    new_tm = tm + timedelta(seconds=(10-tm.second%10)%10)
    date_range = pd.date_range(start=new_tm,periods=433,freq='10min')
    return date_range

###### Particulars functions for creating new features based on Omni data
...
###### Function for global preprocessing of OMNI data
...
def extract_features_from_omni(data_omni_init ):
    ...
    all_features = {**features_mean,**features_last_7}
    
    return all_features

###### Function to create the whole dataframe and its target in training mode
...
########### Functions for propagation of Orbit
